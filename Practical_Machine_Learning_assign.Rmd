...
The Accuracy for rpart model is 0.51 which is very low, so we will check randomForest model.

```{r cache=FALSE}

model2 <- train(har_val$classe ~. , data=har_val,method="rf",prox=TRUE,ntree=100)
model2
```
...
**Conclusion:-** Accuracy in randomForest model shows 0.98.
...
```{r }
test_pred <- predict(mod2, HAR_TEST)
test_pred
table(test_pred)
```
...
##References
- Predictive Modeling with R and the caret Package, Max Kuhn, Ph.D (http://www.edii.uclm.es/~useR-2013/Tutorials/kuhn/user_caret_2up.pdf)
